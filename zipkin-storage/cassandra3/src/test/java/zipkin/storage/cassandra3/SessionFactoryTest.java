...
import static zipkin.storage.cassandra3.SessionFactory.Default.buildCluster;
import static zipkin.storage.cassandra3.SessionFactory.Default.parseContactPoints;
...
public class SessionFactoryTest {
  ...
  public void connectPort_singleContactPoint() {
    assertThat(buildCluster(CassandraStorage.builder().contactPoints("1.1.1.1:9142").build())
  ...
  public void connectPort_whenContactPointsHaveSamePort() {
    assertThat(
        buildCluster(CassandraStorage.builder().contactPoints("1.1.1.1:9143,2.2.2.2:9143").build())
            .getConfiguration().getProtocolOptions().getPort())
        .isEqualTo(9143);
  }
  ...
  public void connectPort_whenContactPointsHaveMixedPorts_coercesToDefault() {
    assertThat(
        buildCluster(CassandraStorage.builder().contactPoints("1.1.1.1:9143,2.2.2.2").build())
            .getConfiguration().getProtocolOptions().getPort())
        .isEqualTo(9042);
  }
  ...
  public void usernamePassword_impliesNullDelimitedUtf8Bytes() {
    Authenticator authenticator =
        buildCluster(CassandraStorage.builder().username("bob").password("secret").build())
  ...
  public void maxConnections_setsMaxConnectionsPerDatacenterLocalHost() {
    PoolingOptions poolingOptions =
        buildCluster(CassandraStorage.builder().maxConnections(16).build())
            .getConfiguration().getPoolingOptions();
